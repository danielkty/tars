...
class DataProto:
    ...
    def split_by_indices(self, indices):
        """
        Creates a new DataProto containing only the data at the specified indices.
        
        Args:
            indices (list or array-like): Indices to select from the DataProto
        ...
        # Handle tensor batch
        if self.batch is not None:
            # Direct indexing of the TensorDict rather than creating a new one with fields
            split_batch = self.batch[indices]
        else:
            split_batch = None
        
        # Handle non-tensor batch
        split_non_tensor_batch = {}
        for k, v in self.non_tensor_batch.items():
            split_non_tensor_batch[k] = v[indices]
        
        # Create new DataProto with the split data
        return DataProto(
            batch=split_batch,
            non_tensor_batch=split_non_tensor_batch,
            meta_info=self.meta_info
        )
